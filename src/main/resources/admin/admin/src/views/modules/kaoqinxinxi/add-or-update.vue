<template>
	<div class="addEdit-block" :style='{"padding":"30px 0 0 0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="select" v-if="type!='info' && !ruleForm.id" label="选择学生" prop="selectedAllocationNo">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="晚归天数" prop="wanguitianshu">
					<el-input v-model="ruleForm.wanguitianshu" placeholder="晚归天数" clearable  :readonly="ro.wanguitianshu"></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="请假天数" prop="qingjiatianshu">
...
export default {
	...
	methods: {
		loadAllocationOptions() {
			this.$http({
				...
				if (data && data.code === 0) {
					this.allocationOptions = (data.data.list || []).map(item => ({
		...
		onAllocationChange(value) {
			const match = (this.allocationOptions || []).find(item => item.value === value);
			if (!match) {
				return;
			}
			const raw = match.label.split(" / ");
			const namePart = raw[0] || "";
			this.ruleForm.xueshengxingming = namePart.replace(/\s*\(.+\)$/, "");
			this.ruleForm.xueshengxuehao = value;
			this.ruleForm.sushemingcheng = raw[1] || "";
			this.ruleForm.susheloudong = raw[2] || "";
			this.ruleForm.fangjianhao = raw[3] || "";
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='wanguitianshu'){
							this.ruleForm.wanguitianshu = obj[o];
							this.ro.wanguitianshu = true;
							continue;
						}
						if(o=='queqintianshu'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../dormmanagesys/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
