<template>
	<div class="main-content" :style='{"padding":"30px 0 0 0"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"180px","margin":"0 0 20px 20px","position":"absolute","zIndex":"1003"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					...
					<div :style='{"margin":"0 0px 15px 0","display":"inline-block"}' class="select" label="宿舍类型" prop="susheleixing">
						<label :style='{"margin":"0 10px 0 0","color":"#666","textAlign":"center","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">宿舍类型</label>
					...
					<div :style='{"margin":"0 0px 15px 0","display":"inline-block"}' class="select" label="有床位" prop="youchuangwei">
						...
						<el-select  @change="youchuangweiChange" clearable v-model="searchForm.youchuangwei" placeholder="请选择有床位">
							<el-option v-for="(item,index) in youchuangweiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid rgba(135, 154, 108, 1)","cursor":"pointer","padding":"0 10px","margin":"0 10px 5px 0","outline":"none","color":"rgba(135, 154, 108, 1)","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('sushexinxi','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"1px solid rgba(135, 154, 108, 1)","cursor":"pointer","padding":"0 10px","margin":"0 10px 5px 0","outline":"none","color":"rgba(135, 154, 108, 1)","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('sushexinxi','分配')" type="success" size="mini" @click="sushefenpeiCrossAddOrUpdateHandler(scope.row,'cross','','','')">分配</el-button>
							<el-button :style='{"border":"1px solid rgba(135, 154, 108, 1)","cursor":"pointer","padding":"0 10px","margin":"0 10px 5px 0","outline":"none","color":"rgba(135, 154, 108, 1)","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('sushexinxi','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"1px solid rgba(135, 154, 108, 1)","cursor":"pointer","padding":"0 10px","margin":"0 10px 5px 0","outline":"none","color":"rgba(135, 154, 108, 1)","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('sushexinxi','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<sushefenpei-cross-add-or-update v-if="sushefenpeiCrossAddOrUpdateFlag" :parent="this" ref="sushefenpeiCrossaddOrUpdate"></sushefenpei-cross-add-or-update>
...
import sushefenpeiCrossAddOrUpdate from "../sushefenpei/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      sushefenpeiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    sushefenpeiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.sushefenpeiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.sushefenpeiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.sushefenpeiCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    refreshDormStats() {
      this.$confirm("确定重算全部宿舍的已住人数和床位状态吗？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$http({
          url: "sushexinxi/refreshDormStats",
          method: "post"
        }).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message({
              message: data.msg || "重算完成",
              type: "success",
              duration: 1500,
              onClose: () => {
                this.getDataList();
