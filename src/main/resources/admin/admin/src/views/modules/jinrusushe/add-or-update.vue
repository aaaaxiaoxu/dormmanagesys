<template>
	<div class="addEdit-block" :style='{"padding":"30px 0 0 0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="select" v-if="type!='info' && !ruleForm.id" label="选择宿舍" prop="selectedDormKey">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="访客姓名" prop="xueshengxingming">
					<el-input v-model="ruleForm.xueshengxingming" placeholder="访客姓名" clearable  :readonly="ro.xueshengxingming"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="访客姓名" prop="xueshengxingming">
					<el-input v-model="ruleForm.xueshengxingming" placeholder="访客姓名" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="访客手机号" prop="xueshengxuehao">
					<el-input v-model="ruleForm.xueshengxuehao" placeholder="访客手机号" clearable  :readonly="ro.xueshengxuehao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="访客手机号" prop="xueshengxuehao">
					<el-input v-model="ruleForm.xueshengxuehao" placeholder="访客手机号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="进入时间" prop="jinrushijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.jinrushijian" 
						type="datetime"
						:readonly="ro.jinrushijian"
						placeholder="进入时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.jinrushijian" label="进入时间" prop="jinrushijian">
					<el-input v-model="ruleForm.jinrushijian" placeholder="进入时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="upload" v-if="type!='info' && !ro.zhaopian" label="照片" prop="zhaopian">
                    <el-image v-if="ruleForm.zhaopian" :src="ruleForm.zhaopian?$base.url + ruleForm.zhaopian:''" style="width:150px;height:150px;"></el-image>
                    <el-button @click="imgAddClick">上传照片</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="upload" v-else-if="ruleForm.zhaopian" label="照片" prop="zhaopian">
					...
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="item" v-for="(item,index) in ruleForm.zhaopian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
			</template>
...
export default {
	...
	created() {
		this.ruleForm.jinrushijian = this.getCurDateTime()
	},
	methods: {
        ...
        imgChange(e){
            this.ruleForm.zhaopian = 'upload/' + e
        },
		buildDormKey(item) {
			return [item.sushemingcheng, item.susheleixing, item.susheloudong, item.fangjianhao].join("||");
		},
		onDormChange(value) {
			const dorm = this.dormOptionsRaw.find(item => this.buildDormKey(item) === value);
			if (!dorm) {
				return;
			}
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jinrushijian'){
							this.ruleForm.jinrushijian = obj[o];
							this.ro.jinrushijian = true;
							continue;
						}
						if(o=='zhaopian'){
				...
				if (data && data.code === 0) {
					...
					if (!this.id) {
						this.$http({
							...
							if (data && data.code === 0) {
								this.dormOptionsRaw = data.data.list || [];
								this.dormOptions = this.dormOptionsRaw.map(item => ({
    ...
    faceMatchHandle() {
        const currentFace = this.normalizeUploadPath(this.ruleForm.zhaopian);
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jinrusushe/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jinrususheCrossAddOrUpdateFlag = false;
						 this.parent.search();
